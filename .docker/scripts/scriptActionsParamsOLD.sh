...
cleanProjectPath() {
    ...
    then
        ...
        cd /var/www
        echo "clean files: "
        find . -maxdepth 1 -type f ! -name "README.md" ! -name ".gitignore"
        find . -maxdepth 1 -type f ! -name "README.md" ! -name ".docker" ! -name ".git" -exec rm {} \;
        
        echo "clean directories: "
        find . -maxdepth 1 -type d ! -name "." ! -name ".docker" ! -name ".git"
        find . -maxdepth 1 -type d ! -name "." ! -name ".docker" ! -name ".git" -exec rm -fr {} \;
...
decompressProject() {
    ...
    then
        ...
        unzip .docker/symfonyFiles/compress/symfony.zip
        initializeSymfonyProject # init symfony project
    fi
}
...
initializeSymfonyProject() {
    ...
    echo "Configure your params from:"

    echo "Generate your JWT data: php bin/console lexik:jwt:generate-keypair"

    echo "Database in .env.local example: DATABASE_URL=postgresql://postgres:postgres@anfitrion.ip.win.machine:54xx/xx_projectName?serverVersion=13&charset=utf8"
    echo "Exec database create: php bin/console doctrine:database:create"
    echo "Exec migration create: php bin/console make:migration"
    echo "Exec migrate migration: php bin/console doctrine:migrations:migrate"
}
...
createProject() {
    ...
    then
        createNewProjectAngular
    ### SYMFONY newProject
    elif [ $TYPE_PROJECT = "s" ] && [ $PROJECTNAME != "symfony" ] 
    then
        createNewProjectSymfony
    ## decompress
    else 
